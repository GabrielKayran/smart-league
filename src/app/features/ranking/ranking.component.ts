...
@Component({
  ...
  standalone: true,
  imports: [MatCardModule, MatTableModule, MatTabsModule, MatIconModule, MatChipsModule, DatePipe],
...
export class RankingComponent {
  ...
  displayedColumns: string[] = [
    'position', 'team', 'points', 'matches', 'wins', 'draws', 'losses',
    'goalsFor', 'goalsAgainst', 'goalDifference', 'form'
  ];

  getPositionClass(position: number): string {
    if (position <= 4) return `position-${position} position-champions`;
    if (position <= 6) return 'position-europa';
    if (position >= 17) return 'position-relegation';
    return '';
  }
